...
export const getCloudwatchAlarms = async (
  params?: DescribeAlarmsInput
): Promise<IAlarmDetail[]> => {
  ...
  do {
    ...
    results = result.CompositeAlarms
      ? results.concat(
          result.CompositeAlarms?.map((alarm) => {
            return {
 ...
 *
 */
export const getAggregateAlarmState = async (
  searchTerms: string[]
): Promise<AggregateAlarmState> => {
  const alarmDetails: IAlarmDetail[] = await getAlarms(searchTerms);

  return {
    alarmDetails,
    state:
      (alarmDetails.find((a) => a.state === AlarmState.ALARM)
        ?.state as AlarmState) ?? AlarmState.OK,
